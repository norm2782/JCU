...
-- We need the right-hand side of the rule, since the left-hand side would
-- bring us back to where we were.
-- In order to solve the right-hand side, we need the environment with
-- variables which map terms like X0 and Y0 to concrete values.
...
showBdg bs (x, t)  | isShowable  = x ++ " = " ++ showTerm t ++ "\n"
                   | otherwise   = ""
